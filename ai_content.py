...
CONTENT_DIR     = os.path.join(os.path.dirname(__file__), "content_resources")

app = FastAPI(title="AI Content & Resource Recommender API")
...
def _call_ollama_with_retries(prompt: str, required_keys: list[str], label: str) -> dict:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if missing:
                print(f"[CONTENT] {label} attempt {attempt}: missing keys {missing}, retrying...")
            ...
            print("[CONTENT] ERROR: Cannot connect to Ollama. Is it running?")
...
def build_focus_context(plan: dict) -> str:
    ...
    for tip in study_plan.get("study_tips", []):
        lines.append(
            f"  • [{tip.get('applies_to','')}] {tip.get('tip','')}"
        )
...
def _build_topic_resources_prompt(topic: str, activity_context: str, focus_context: str) -> str:
...
def generate_topic_resources(topics: list[str], activity_ctx: str, focus_ctx: str) -> list[dict]:
...
def generate_daily_resources(plan: dict) -> dict:
    ...
    result = _call_ollama_with_retries(
        prompt, ["daily_resources"], "DailyResources"
    )
    return result
...
async def create_resource_recommendations(
    ...
    print(f"[CONTENT] Generating resources for {student_id} "
...
async def list_saved_resources(
    ...
    return {"student_id": student_id, "total_files": len(resources), "resources": resources}
...
if __name__ == "__main__":
    import uvicorn
    print(f"[CONTENT] Using Ollama model: {OLLAMA_MODEL} at {OLLAMA_BASE_URL}")
    uvicorn.run("ai_content:app", host="0.0.0.0", port=8003, reload=True)
