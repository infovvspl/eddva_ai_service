...
# CONFIGURATION
# ─────────────────────────────────────────────────────────────
screenshot_dir = r"D:\Qwegle\Qwegle\Cheating_Detection\cheating_screenshots"
...
GAZE_UP_MAX_DURATION     = 3.0   # seconds – pupil looking up = reading something above
...
gaze_off_frames         = 0
gaze_up_start_time      = None   # tracks when upward gaze started
head_pose_frames        = 0
...
def detect_iris(eye_lm, face_lm, w, h):
    ...
    top_anchor = min(u_y, l_y)
    v_ratio = (cy - top_anchor) / eye_height if eye_height > 1 else 0.5
    v_ratio = max(0.0, min(1.0, v_ratio))

    H_LEFT_THRESH  = 0.35
    H_RIGHT_THRESH = 0.65
    V_UP_THRESH    = 0.35   # iris in top 35% of eye height = looking Up
...
def draw_hud(frame, blinks, cheat_count, cheat_pct, attention,
             ...
             partial_yaw_frames, mouth_activity_pct, current_frame, total_vid_frames,
             up_dur_str):
    """Overlay all stats onto the frame."""
    overlay = frame.copy()
    cv2.rectangle(overlay, (0, 0), (410, 360), (0, 0, 0), -1)
    ...
    progress_pct = (current_frame / total_vid_frames * 100) if total_vid_frames > 0 else 0
    gaze_up_col  = (0, 0, 255) if up_dur_str else (0, 0, 255)
    ...
    lines = [
        ...
        (f"Gaze L={gaze_l} R={gaze_r}{up_dur_str}",                              (0, 0, 255) if up_dur_str else (0, 0, 255)),
        ...
        (f"Video: {current_frame}/{total_vid_frames} ({progress_pct:.0f}%)",      (180, 180, 180)),
    ]
...
# ─────────────────────────────────────────────────────────────
# VIDEO INPUT — change this path to your video file
# ─────────────────────────────────────────────────────────────
VIDEO_PATH    = r"D:\VVSPL\AI Study Planner\Cheating\WIN_20260218_14_49_03_Pro.mp4"
PLAYBACK_FPS  = 30   # target FPS for display — increase to speed up, decrease to slow down
...
if not cap.isOpened():
    sys.stdout.write(f"ERROR: Could not open video file: {VIDEO_PATH}\n")
    sys.exit(1)

video_fps    = cap.get(cv2.CAP_PROP_FPS) or 30
total_video_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
wait_ms      = max(1, int(1000 / PLAYBACK_FPS))
    ...
    while cap.isOpened():
        ...
        if result.multi_face_landmarks:
            ...
            if blink_state_v == "open":
                ...
                # Up gaze sustained > 5s = reading hidden notes above camera
                both_up = (gaze_l_str == "Up" and gaze_r_str == "Up")
                if both_up:
                    if gaze_up_start_time is None:
                        gaze_up_start_time = time.time()
                    else:
                        up_duration = time.time() - gaze_up_start_time
                        if up_duration >= GAZE_UP_MAX_DURATION:
                            cheating_this_frame = True
                            reasons.append((f"Looking up {up_duration:.1f}s (reading notes?)", "gaze_up"))
                else:
                    gaze_up_start_time = None   # reset if gaze leaves Up
            else:
                gaze_l_str         = gaze_r_str = "–"
                gaze_off_frames    = 0
        ...
        up_dur_str        = f"  UP:{time.time()-gaze_up_start_time:.1f}s" if gaze_up_start_time else ""
        draw_hud(frame, blink_count, cheating_count, cheating_pct, attention_score,
                 ...
                 current_vid_frame, total_video_frames, up_dur_str)
